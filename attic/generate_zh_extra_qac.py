...
#
# The Chinese coverage run: +40 QAC rows in two phases. Phase A forced the query
# language to zh on 20 uncovered publications (10 technical + 10 semantic), regardless
# of source language, tagged strategy `forced_zh` with sentinel strategy code 0.
# Phase B drew 20 more from publications that already had a zh corpus row and let the
# normal strategy logic pick the query language. Generation on gpt-5-mini, grading on
# OpenRouter Claude Sonnet 4.6 with extended thinking so the rows were directly
# appendable to the already-regraded qac_chempatents{,_best}.csv.
#
# Used:     May-June 2026 (added 2026-05-02, last run 2026-06-11)
# Retired:  a one-off top-up script with the whole generate+grade+append engine inlined; the
# next coverage run (generate_extra_qac.py) had to import from it, which is how you
# know it should have been a library.
# Replaced by: clir_bench.core.qagen (the engine) driven by the 'coverage' plan in
# domains/chem_patents/qac/plans.py
#
# This file is a verbatim copy from the old repo (Multi-Lingual-QAC),
# original path: scripts/generate_zh_extra_qac.py
