...
#
# Re-graded existing QAC candidates without regenerating them: read an _all_generated
# CSV, re-ran the faithfulness and mode-specific quality verifiers through OpenRouter
# on Claude Sonnet 4.6 with extended thinking (8k budget, 12k max tokens) instead of
# the OpenAI reasoning_effort path, recomputed the overall/total scores and wrote a
# CSV in exactly the same schema.
#
# Used:     April-June 2026 (added 2026-04-22, last run 2026-06-10) - this produced the
# regraded qac_chempatents files that later runs had to stay compatible with
# Retired:  regrading is a routine operation, not a one-off; it deserved a command with
# arguments rather than a script with constants.
# Replaced by: `clir qac regrade`
#
# This file is a verbatim copy from the old repo (Multi-Lingual-QAC),
# original path: scripts/regrade_with_openrouter.py
