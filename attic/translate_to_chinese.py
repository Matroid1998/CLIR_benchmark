...
#
# Machine-translated selected Google Patents publications into simplified Chinese with
# gpt-5.5 (title + abstract only), rebuilt the `context` field with the dataloader's
# English structural labels, and appended the new zh rows to the corpus CSV in place.
# Skipped publications that already had a zh row.
#
# Used:     May-June 2026 (added 2026-05-02, last run 2026-06-08), to give the corpus enough
# Chinese coverage to generate zh queries against
# Retired:  it machine-translates SOURCE documents. Only human-translated patent versions are
# admissible as corpus text - a retrieval benchmark whose gold documents are MT
# output measures the translator as much as the retriever. MT remains fine for the
# GENERATED questions and answers; the rule is about source documents only.
# Replaced by: nothing - deliberately not ported. Chinese coverage now comes from genuine
# multilingual publications in the ingested sources.
#
# This file is a verbatim copy from the old repo (Multi-Lingual-QAC),
# original path: scripts/translate_to_chinese.py
