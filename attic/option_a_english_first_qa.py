...
#
# Option A of the QAC pipeline: sample the corpus, generate ONE English Q&A pair per
# document with gpt-5-mini, run three English gate checks (language / faithfulness /
# quality) with retry-and-feedback loops, then translate the approved pair into 15
# languages one language at a time, each translation gated by its own 8-dimension
# quality checker. Wrote a flat qac.csv of (corpus_id, language, question, answer).
#
# Used:     March 2026 (first QAC prototype; added 2026-03-10, last touched 2026-03-18)
# Retired:  English-first generate-then-translate makes every non-English query a translation
# artifact of an English query - the translation checker in here exists precisely
# because that is hard to get right. Option B (generate directly in the target
# language from multilingual passages) removed the problem instead of policing it.
# Replaced by: direct multilingual generation - clir_bench.core.qagen plus the per-language
# generation prompts in domains/chem_patents/qac/prompts/generation/
#
# This file is a verbatim copy from the old repo (Multi-Lingual-QAC),
# original path: src/multi_lingual_qac/qac_generation/openai_qa.py
