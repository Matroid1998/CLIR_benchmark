...
limited-disk "download -> process -> discard" constraint), and builds a
...
def cache_dir(context: AppContext) -> Path:
    """Where the OBO release, the parsed graph and the name cache live."""
    return context.workspace.data("chebi")


def variant_for(context: AppContext, requested: Optional[str] = None) -> str:
    """Release variant: explicit flag, else the domain default."""
    return str(requested or context.setting("chebi_variant", "full"))
...
def _cache_path(directory: Path, variant: str) -> Path:
    return directory / f"chebi_{variant}_graph.json.gz"
...
def load_chebi_graph(
    ...
    *,
    ...
    (str), ``synonyms`` (list[str]), ``brand_names`` (list[str]), ``wiki_en``
    (str | None).
    """
    directory = Path(directory)
    cache = _cache_path(directory, variant)
    if cache.exists() and not refresh:
        graph, _ = _read_cache(cache)
        print(f"Loaded cached ChEBI {variant} graph: {graph.number_of_nodes()} nodes")
        return graph

    obo_gz = directory / _variant_filename(variant)
