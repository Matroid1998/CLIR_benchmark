...
# Kana, CJK ideographs (ext-A + unified), Hangul syllables, CJK compatibility.
...
def scan_corpus(
    ...
    *,
    ...
    ``field`` is read directly rather than through ``schema.text_of`` on
    purpose: the published concept sets were built by matching against the
    assembled ``context`` column only, and falling back to abstract/title would
    silently change which documents are gold for a concept.

    Returns:
      * ``concept_to_docs``: ``{doc_id: ...}`` keyed by chebi id
    ...
    for row in rows:
        ...
        doc_lang = schema.language_of(row)
