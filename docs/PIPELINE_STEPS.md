...
This document is a historical record of the pipeline as built in the
`Multi-Lingual-QAC` repo, and its content is unchanged. The steps still describe
what the pipeline does, but the commands, flags and module paths named above no
longer exist in this form — everything now runs through the `clir` CLI. See
[MIGRATION.md](MIGRATION.md) for the old-flag to new-command mapping.

Step 9 in particular describes Option A (generate in English, then translate),
which was superseded by direct multilingual generation; the original script is
archived at `attic/option_a_english_first_qa.py`.
